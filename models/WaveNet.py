...
class WaveNet(nn.Module):
    ...
    def __init__(self, num_blocks=2, num_layers=4, num_classes=10, output_len=16, ch_start=16, ch_residual=32,
                 ch_dilation=32, ch_skip=32, ch_end=32, kernel_size=3, bias=True, causal=False) -> None:
        super().__init__()

        """Parameters"""
        self.num_blocks = 1 # num_blocks
        self.num_layers = 10 #num_layers
        self.num_classes = num_classes
        self.output_len = output_len

        self.ch_start = 64 #ch_start
        self.ch_residual = ch_residual
        self.ch_dilation = ch_dilation
        self.ch_skip = ch_skip
        self.ch_end = 10 #ch_end
        self.kernel_size = kernel_size
        self.bias = bias
        self.causal = False #causal
...
if __name__ == '__main__':
    ...
    ch = 640
    length = 64
    input = torch.randn((batch, ch, length))
    print(input.shape)
